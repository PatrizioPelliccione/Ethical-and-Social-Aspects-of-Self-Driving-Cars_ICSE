...
%\ins{Could the \enquote{character}, such as \enquote{aggressiveness} of a self-driving car be a criterion distinguishing between different cars? Will it be possible for a car to drive in the style of a racing driver? Will some cars be allowed to prioritize its passengers lives over all other participants in the traffic? It is worth to notice that this would be a false promise, as \enquote{trolley problem} presupposes that car indeed can predict and act perfectly, which we already argued is not the case in real life.}
